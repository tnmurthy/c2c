...
import { Plus, Search, Filter, MoreHorizontal, Loader2, Download, Star } from 'lucide-react';
...
interface Opportunity {
  ...
  amount: number;
  currency: string;
  candidate_id?: string | null; // Added relation column
  accounts?: {
    name: string;
  };
  candidate?: Candidate; // Attached from loaded talent pool
}
...
export default function OpportunitiesPage() {
  ...
  const [candidates, setCandidates] = useState<Candidate[]>([]); // Added state to store candidates
                          ...
                          <div className="mb-3 pt-3 border-t border-slate-800/60">
                            ...
                            <div className="flex flex-wrap gap-1 mb-3">
                               {['IQ', 'EQ', 'AQ', 'SQ', 'SpQ'].map(dim => {
                                 const val = opp.candidate!.scores?.[dim as keyof CandidateScore];
